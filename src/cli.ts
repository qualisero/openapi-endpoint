...
interface OpenAPISpec {
  /** OAS extension: set to `true` by the spec producer when dump-direction `required` is fully encoded in the spec. */
 ...
 * wrapped in RequireAll) and bare aliases with @deprecated tags.
 */
...
${responsesNsEntries}
  ...
  // Only emit the RequireAll import when the Responses namespace uses it:
  // an unused import fails consumer builds with noUnusedLocals (TS6133), which
  // /* eslint-disable */ does not cover.
  const requireAllImport =
    responseSchemas.length > 0 ? "import type { RequireAll } from '@qualisero/openapi-endpoint'\n" : ''
...
/**
 * Detects the x-direction-finalized root marker and logs an advisory notice.
 *
...
async function generateForSpec(openapiInput: string, outputDir: string, opts: ResolvedOptions): Promise<void> {
  ...
  // Generate all files
  await Promise.all([
    // Clone: openapiTS (Redocly) may keep and mutate the parsed spec by reference,
    // while the sibling generators below read the original concurrently.
    generateTypes(structuredClone(openApiSpec), outputDir, defaultNonNullable),
...
function loadCodegenConfig(configPath: string): CodegenConfig {
  ...
  if (cfg.options !== undefined) {
    validateConfigOptions(cfg.options, `${configPath}: options`)
  }
  for (let i = 0; i < cfg.specs.length; i++) {
    ...
    validateConfigOptions(spec, `${configPath}: specs[${i}]`)
  }
  return parsed as CodegenConfig
}

/**
 * Validate option values in a config "options" block or per-spec entry.
 * Throws on invalid values instead of silently generating with defaults
 * (e.g. "enumCase": "Pascal" would otherwise fail every === 'const' check
 * and silently produce pascal output).
 */
function validateConfigOptions(obj: unknown, context: string): void {
  if (typeof obj !== 'object' || obj === null) {
    throw new Error(`${context}: must be a JSON object`)
  }
  const o = obj as Record<string, unknown>
  if (o.enumCase !== undefined && o.enumCase !== 'pascal' && o.enumCase !== 'const') {
    throw new Error(`${context}.enumCase: must be 'pascal' or 'const' (got ${JSON.stringify(o.enumCase)})`)
  }
  if (o.defaultNonNullable !== undefined && typeof o.defaultNonNullable !== 'boolean') {
    throw new Error(`${context}.defaultNonNullable: must be a boolean (got ${JSON.stringify(o.defaultNonNullable)})`)
  }
  if (o.useStrictResponse !== undefined && typeof o.useStrictResponse !== 'boolean') {
    throw new Error(`${context}.useStrictResponse: must be a boolean (got ${JSON.stringify(o.useStrictResponse)})`)
  }
  if (o.excludePrefix !== undefined && typeof o.excludePrefix !== 'string' && o.excludePrefix !== false) {
    throw new Error(`${context}.excludePrefix: must be a string or false (got ${JSON.stringify(o.excludePrefix)})`)
  }
  if (
    o.emitValueSchemas !== undefined &&
    o.emitValueSchemas !== 'request' &&
    o.emitValueSchemas !== 'all' &&
    o.emitValueSchemas !== false
  ) {
    throw new Error(
      `${context}.emitValueSchemas: must be 'request', 'all', or false (got ${JSON.stringify(o.emitValueSchemas)})`,
    )
  }
}
