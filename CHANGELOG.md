...
- Codegen warnings for skipped operations: a `requestBody` (or, in `all` mode, a 2xx response with content) without an inline `application/json` schema — e.g. `$ref`-valued bodies or `application/json; charset=utf-8` media types — now prints a warning instead of being silently skipped. Only schemas declared inline under the exact `application/json` media type are emitted.
